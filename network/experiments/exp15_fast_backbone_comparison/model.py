"""Model with configurable backbone for fast backbone comparison.

This model supports multiple lightweight backbones:
- RepVGG-A0 (timm): Re-parameterizable VGG-style
- MobileOne-S0 (timm): Apple's fast backbone
- ShuffleNetV2_x0.5 (torchvision): Channel shuffle based

The architecture follows exp13's rotation correlation approach but allows
swapping the backbone for speed comparisons.
"""
...
class RotationCorrelationModule(nn.Module):
    ...
    def _extract_region(
        ...
        x_idx = (position[:, 0] >= 0.5).long()
        y_idx = (position[:, 1] >= 0.5).long()
...
if __name__ == "__main__":
    print("=" * 60)
    print("Fast Backbone Model Test")
    print("=" * 60)
