"""Training script for MobileViT-XS backbone experiment.

This experiment tests whether the MobileViT-XS backbone (a hybrid CNN-Transformer
architecture) improves puzzle piece localization compared to MobileNetV3-Small.

Key changes from exp13:
- Backbone: MobileNetV3-Small -> MobileViT-XS
- Feature dimension: 576 -> 384
- Total parameters: 5.27M -> 4.43M (-16%)

Target: Match or exceed exp13's performance (86% quad, 93% rot)
"""
...
def train_epoch(
    ...
    for step_idx, (pieces, puzzles, targets, quadrants, rotations) in enumerate(loader):
        pieces = pieces.to(device)
        puzzles = puzzles.to(device)
        targets = targets.to(device)
    ...
    # Handle remaining gradients if loader length not divisible by accumulation_steps
...
def run_experiment(  # noqa: C901
    n_train_puzzles: int = 4499,
    n_test_puzzles: int = 500,
    epochs: int = 100,
    batch_size: int = 16,
    ...
    Args:
        ...
        batch_size: Training batch size (reduced for memory, use accumulation).
        accumulation_steps: Gradient accumulation steps (effective_batch = batch_size * accumulation_steps).
    ...
    print("\n" + "=" * 70)
    print("EXPERIMENT 14: MobileViT-XS BACKBONE")
    print("=" * 70)
    print("Testing MobileViT-XS (hybrid CNN-Transformer) as backbone:")
    print("  - Backbone: MobileViT-XS (was MobileNetV3-Small in exp13)")
    print("  - Feature dim: 384 (was 576)")
    print("  - Parameters: ~4.4M (was 5.3M)")
    print("-" * 70)
    ...
    print(f"Batch size: {batch_size} (effective: {effective_batch_size} with {accumulation_steps}x accumulation)")
    ...
    print("\nModel: DualInputRegressorWithRotationCorrelation (MobileViT-XS)")
    ...
    print(f"  Exp12: {exp12_quad:.1%}")
    print(f"  Exp13: {test_final['quadrant_accuracy']:.1%}")
    quad_vs_exp12 = test_final["quadrant_accuracy"] - exp12_quad
    print(f"  vs Exp12: {quad_vs_exp12:+.1%}")

    print("\nRotation Accuracy:")
    print(f"  Exp10: {exp10_rot:.1%}")
    print(f"  Exp11: {exp11_rot:.1%}")
    print(f"  Exp12: {exp12_rot:.1%}")
    print(f"  Exp13: {test_final['rotation_accuracy']:.1%}")
    rot_vs_exp12 = test_final["rotation_accuracy"] - exp12_rot
    print(f"  vs Exp12: {rot_vs_exp12:+.1%}")
    ...
    results: dict[str, Any] = {
        "experiment": "exp14_mobilevit_backbone",
        "n_train_puzzles": n_train_puzzles,
        "n_test_puzzles": n_test_puzzles,
        ...
        "comparison": {
            ...
            "exp12_quadrant_accuracy": exp12_quad,
            "exp12_rotation_accuracy": exp12_rot,
            "quadrant_improvement_vs_exp12": quad_vs_exp12,
            "rotation_improvement_vs_exp12": rot_vs_exp12,
        },
        "success_criteria": {
...
def main(
    ...
    Args:
        ...
        batch_size: Training batch size (reduced for memory).
        accumulation_steps: Gradient accumulation steps (effective_batch = batch_size * accumulation_steps).
    ...
    exp12_quad = 0.665
    exp12_rot = 0.869
    quad_diff = quad_acc - exp12_quad
    rot_diff = rot_acc - exp12_rot
    print("Comparison with exp12 (800 puzzles):")
    print(f"  Position: {quad_diff:+.1%}")
    print(f"  Rotation: {rot_diff:+.1%}")
    if quad_diff > 0.05:
        print("Position improved with more data - hypothesis confirmed!")
    elif quad_diff < -0.05:
        print("Position regressed - more data didn't help as expected.")
    else:
        print("Position roughly stable.")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=16,
        help="Training batch size (default: 16, reduced for MobileViT memory)",
    )
    parser.add_argument(
        "--accumulation-steps",
        type=int,
        default=4,
        help="Gradient accumulation steps (effective_batch = batch_size * accumulation_steps, default: 4)",
    )
    parser.add_argument(
        "--backbone-lr",
        type=float,
