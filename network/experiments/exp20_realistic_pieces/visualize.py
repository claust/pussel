"""Visualization utilities for 4x4 grid experiment with realistic pieces.

Provides functions for visualizing:
- Cell predictions on a 4x4 grid
- Cell confusion matrix (16x16)
...
def save_prediction_grid(
    ...
    Args:
        ...
        pred_cells: List of predicted cell indices (0-15).
        true_cells: List of true cell indices (0-15).
    ...
    # Plot 1: Position scatter plot with 4x4 grid
    ax1 = axes[0]
    ...
    # Add text annotations for non-zero values (smaller font for 16x16)
    ...
    fig.suptitle(
        f"Exp20: 4x4 Grid (Realistic Pieces) - Cell: {cell_acc:.1%}, Rot: {rot_acc:.1%}",
        fontsize=14,
    )
...
def save_training_curves(
    ...
    """Save training curves visualization for 4x4 grid experiment.
    ...
    ax3.axhline(y=50, color="green", linestyle="--", alpha=0.7, label="Target (50%)")
    ...
    ax4.axhline(y=70, color="green", linestyle="--", alpha=0.7, label="Target (70%)")
    ...
    plt.suptitle("Exp20: 4x4 Grid (Realistic Pieces) Training Curves", fontsize=14)
    plt.tight_layout()
    plt.savefig(output_path, dpi=150, bbox_inches="tight")
    plt.close()
...
if __name__ == "__main__":
    print("Visualization utilities for exp20_realistic_pieces")
