"""Dual-input regressor with position AND rotation prediction.

This model extends exp9's DualInputRegressorWithCorrelation to predict
both position (cx, cy) and rotation (0, 90, 180, 270 degrees).

Multi-task learning approach:
- Position: Spatial correlation -> regression (same as exp9)
- Rotation: Piece features -> 4-class classification (new head)

The rotation head only looks at the piece features since:
1. Rotation is a property of the piece, not its position
2. The piece appearance changes with rotation, but the puzzle doesn't
"""
...
class DualInputRegressorWithRotation(nn.Module):
    """Dual-input model with position AND rotation prediction.

    Multi-task architecture:
    - Position: Spatial correlation (same as exp9)
    - Rotation: Classification head on piece features (new)
    """
    ...
    def __init__(
        ...
        # NEW: Rotation classification head
        # Takes piece features and predicts rotation (4 classes)
        self.rotation_head = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Dropout(dropout),
            nn.Linear(64, 4),  # 4 rotation classes: 0, 90, 180, 270
        )
    ...
    def forward(self, piece: torch.Tensor, puzzle: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        # Rotation prediction from piece features only
...
if __name__ == "__main__":
    print("=" * 60)
    print("Model Architecture Test - Position + Rotation Prediction")
    print("=" * 60)
    ...
    model = DualInputRegressorWithRotation(freeze_backbone=False)
    ...
    print(f"Rotation head parameters: {rotation_params:,}")
