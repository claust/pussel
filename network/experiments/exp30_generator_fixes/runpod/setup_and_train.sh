#!/bin/bash
# exp30 RunPod setup + training.
# Run after extracting runpod_training.tar.gz into /workspace.
#
# Steps: install deps -> extract source puzzles -> generate the lossless
# RGBA piece dataset (parallel, resumable) -> train with the unchanged
# exp26 recipe on square-framed pieces, selecting the checkpoint on val and
...
# Note the _v2 suffix: these pieces are NOT interchangeable with exp26's
# (the base rotation is applied losslessly), so they get their own root.
RGBA_DIR="${RGBA_DIR:-/root/realistic_4x4_rgba_v2}"

echo "========================================"
echo "exp30 RunPod Setup (preset=$AUG_PRESET, epochs=$EPOCHS, batch=$BATCH_SIZE)"
echo "========================================"
...
# opencv-python-headless is new for exp30: data_prep.largest_component_bbox
# (the real path's bbox geometry) needs cv2.
