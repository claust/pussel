"""Training entry point for exp30: exp26's recipe on de-shortcut data.

The training recipe is **exp26, unchanged** — same model
(``FastBackboneModel`` / ShuffleNetV2_x0.5), same optimizer and LRs, same
``AugmentConfig`` defaults and ablation presets, same frozen exp20 split,
same harness (val selects the checkpoint, the synthetic test set is touched
once with ``--eval-test``), same output artifact names so
``exp25_north_star_eval/evaluate.py`` consumes
``checkpoint_best_state_dict.pt`` with no changes.

The only difference is the data: pieces come from the lossless generator
(``datasets/realistic_4x4_rgba_v2``) and are square-framed with the real
path's 8% margin before augmentation, so the border-touch and half-pixel
blur rotation shortcuts no longer exist and training framing matches
deployment framing.

Prediction under test: north_star **rotation accuracy jumps well above
33.5%** and the 90deg/270deg prediction bias disappears.

Run from the network/ directory:
    uv run python -m experiments.exp30_generator_fixes.train --epochs 50
    uv run python -m experiments.exp30_generator_fixes.train --aug-preset no_photometric --epochs 50
"""
...
from .framed_dataset import GRID_SIZE, NUM_CELLS, create_datasets_from_split

DEFAULT_OUTPUT_DIR = Path(__file__).parent / "outputs" / "generator_fixes"
...
def main(args: argparse.Namespace) -> dict[str, Any]:
    """Train exp30 on the de-shortcut dataset with val-based selection.
    ...
    print("\n" + "=" * 70)
    print("EXP30 GENERATOR FIXES (exp26 recipe, lossless rotation + real framing)")
    print("=" * 70)
    ...
    results: dict[str, Any] = {
        "experiment": "exp30_generator_fixes",
        ...
        "framing": "lossless_transpose + pad_to_square(margin=0.08)",
...
def parse_args() -> argparse.Namespace:
    """Parse CLI arguments."""
    parser = argparse.ArgumentParser(description="exp30 generator-fix training (frozen split, val selection)")
    ...
    parser.add_argument("--dataset-root", type=Path, default=None, help="RGBA pieces dataset root (v2)")
