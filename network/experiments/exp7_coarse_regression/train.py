"""Training script for coarse regression experiment.

This experiment tests whether cross-puzzle generalization is achievable
with a simplified 2x2 quadrant prediction task using coordinate regression.

Phase 1 (this script): Train with frozen MobileNetV3 backbone.
Phase 2 (future): Fine-tune the backbone for improved performance.
...
def run_experiment(
    ...
    """Run the coarse regression experiment.
    ...
    """
    print("\n" + "=" * 70)
    print("EXPERIMENT 7: COARSE REGRESSION (2x2 QUADRANTS)")
    print("=" * 70)
    ...
    results: dict[str, Any] = {
        "experiment": "exp7_coarse_regression",
...
def main(
    ...
    if test_acc < random_baseline * 1.2:  # Less than 30%
        ...
        print("The coarse regression task may still be too difficult.")
        ...
        print("Consider Phase 2 (fine-tuning) or architectural changes.")
    elif test_acc < 0.70:  # 50-70%
        print("Test accuracy significantly above random (50-70%).")
        print("CONCLUSION: Meaningful generalization achieved!")
        print("Proceed with Phase 2 fine-tuning for improvement.")
    else:  # 70%+
        print(f"Test accuracy high ({test_acc:.1%})!")
        print("CONCLUSION: Strong generalization achieved!")
        print("The coarse regression approach works well.")
        print("Next: Increase grid resolution (3x3, 4x4, etc.)")

    return results


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Coarse regression experiment (2x2 quadrants)")
    parser.add_argument(
        "--epochs",
        type=int,
        default=100,
