...
- Puzzle Encoder: MobileNetV3-Small → spatial feature map (8x8x576)
...
class DualInputRegressor(nn.Module):
    """Original dual-input model WITHOUT spatial correlation.

    DEPRECATED: This model discards spatial information from the puzzle,
    limiting its ability to match pieces to locations. Use
    DualInputRegressorWithCorrelation instead.
    """
    ...
    def __init__(
        ...
        """Initialize the dual-input regressor."""
        super().__init__()
        self.freeze_backbone = freeze_backbone
        self.share_backbone = share_backbone

        weights = MobileNet_V3_Small_Weights.IMAGENET1K_V1
        self.piece_backbone = models.mobilenet_v3_small(weights=weights)

        if share_backbone:
            self.puzzle_backbone = self.piece_backbone
        else:
            self.puzzle_backbone = models.mobilenet_v3_small(weights=weights)

        self.piece_backbone.classifier = nn.Identity()  # type: ignore[assignment]
        if not share_backbone:
            self.puzzle_backbone.classifier = nn.Identity()  # type: ignore[assignment]
    ...
    def forward(self, piece: torch.Tensor, puzzle: torch.Tensor) -> torch.Tensor:
        """Forward pass."""
        piece_feat = self.piece_backbone(piece)
        puzzle_feat = self.puzzle_backbone(puzzle)
        combined = torch.cat([piece_feat, puzzle_feat], dim=1)
    ...
    def predict_quadrant(self, piece: torch.Tensor, puzzle: torch.Tensor) -> torch.Tensor:
        """Predict quadrant."""
        position = self.forward(piece, puzzle)
        cx, cy = position[:, 0], position[:, 1]
        quadrant = (cx >= 0.5).long() + 2 * (cy >= 0.5).long()
        return quadrant
...
if __name__ == "__main__":
    ...
    print("\n--- New Model: With Spatial Correlation ---")
    ...
    print("\n--- Old Model: Without Spatial Correlation (DEPRECATED) ---")
    old_model = DualInputRegressor(freeze_backbone=True)
    old_params = count_parameters(old_model, trainable_only=True)
    print(f"Trainable parameters: {old_params:,}")
