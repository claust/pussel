...
This module addresses the second issue from exp10: memorization due to 16x
exposure per puzzle (4 quadrants x 4 rotations).

Key changes from exp10:
- Exp10: All 4 rotations for each quadrant = 16 samples per puzzle
- Exp11: Random 1 rotation per quadrant = 4 samples per puzzle

This maintains the same data diversity as exp9 (4 samples per puzzle) while
still teaching the model to predict rotation. The rotation for each sample
is randomly selected at each epoch, ensuring the model sees varied rotations
...
class QuadrantRandomRotationDataset(Dataset):
    ...
    2. Provides rotation diversity across epochs (model sees all rotations eventually)
    ...
    def __init__(
        ...
        # Each puzzle contributes 4 quadrants = 4 samples (rotation is random)
...
if __name__ == "__main__":
    print("Testing QuadrantRandomRotationDataset...")
    ...
    # Test train dataset (random rotations)
    ...
    # Show rotation varies per access
    print("\nRotation randomness check (same sample, 5 accesses):")
    for i in range(5):
        _, _, _, _, rotation = train_dataset[0]
        print(f"  Access {i + 1}: Rotation = {ROTATION_ANGLES[int(rotation.item())]}deg")

    # Test test dataset (all rotations)
    print("\n--- Test Dataset (All Rotations) ---")
    print("First 16 samples from one puzzle:")
    for i in range(16):
        _, _, _, q, r = test_dataset[i]
        print(f"  Sample {i}: Quadrant {q.item()}, " f"Rotation {ROTATION_ANGLES[int(r.item())]}deg")
