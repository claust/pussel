"""Visualization utilities for spatial rotation head experiment.
...
def save_prediction_grid(
    ...
    fig.suptitle(
        f"Exp11 Spatial Rotation - Quad: {quad_acc:.1%}, Rot: {rot_acc:.1%}",
        fontsize=14,
    )
...
def save_training_curves(
    ...
    ax4.axhline(y=61, color="orange", linestyle=":", alpha=0.7, label="Exp10 (61%)")
    ax4.set_xlabel("Epoch")
    ax4.set_ylabel("Accuracy (%)")
    ax4.set_title("Rotation Accuracy (vs Exp10)")
    ax4.legend()
    ax4.grid(True, alpha=0.3)
    ax4.set_ylim(0, 105)

    plt.suptitle("Exp11: Spatial Rotation Head Training Curves", fontsize=14)
    plt.tight_layout()
    plt.savefig(output_path, dpi=150, bbox_inches="tight")
    plt.close()
...
if __name__ == "__main__":
    print("Visualization utilities for exp11_spatial_rotation")
