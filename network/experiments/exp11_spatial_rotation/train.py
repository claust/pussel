"""Training script for Spatial Rotation Head experiment.

This experiment fixes two issues from exp10:
1. Architectural: Use SpatialRotationHead (preserves spatial features)
2. Data: Use random rotation sampling (4 samples/puzzle vs 16)
...
from .model import DualInputRegressorWithSpatialRotation, count_parameters
...
def compute_metrics(
    model: DualInputRegressorWithSpatialRotation,
...
def run_experiment(  # noqa: C901
    ...
    """Run the spatial rotation head experiment.
    ...
    """
    print("\n" + "=" * 70)
    print("EXPERIMENT 11: SPATIAL ROTATION HEAD")
    print("=" * 70)
    print("Key changes from exp10:")
    print("  1. Spatial rotation head (preserves feature map structure)")
    print("  2. Random rotation sampling (4 samples/puzzle, not 16)")
    print("-" * 70)
    ...
    # Create datasets with random rotation for training
    print("\nLoading datasets...")
    ...
    # Create model with spatial rotation head
    model = DualInputRegressorWithSpatialRotation(
    ...
    print("\nModel: DualInputRegressorWithSpatialRotation (MobileNetV3-Small)")
    print(f"Total parameters: {total_params:,}")
    print(f"Trainable parameters: {trainable_params:,}")

    # Count spatial rotation head params
    ...
    print("\n" + "=" * 50)
    print("COMPARISON WITH EXP10")
    print("=" * 50)
    exp10_quad = 0.784
    exp10_rot = 0.614
    print(f"Exp10 test quadrant accuracy: {exp10_quad:.1%}")
    print(f"Exp11 test quadrant accuracy: {test_final['quadrant_accuracy']:.1%}")
    quad_improvement = test_final["quadrant_accuracy"] - exp10_quad
    print(f"Quadrant improvement: {quad_improvement:+.1%}")
    print()
    print(f"Exp10 test rotation accuracy: {exp10_rot:.1%}")
    print(f"Exp11 test rotation accuracy: {test_final['rotation_accuracy']:.1%}")
    rot_improvement = test_final["rotation_accuracy"] - exp10_rot
    print(f"Rotation improvement: {rot_improvement:+.1%}")
    ...
    print(f"Position target (>90%):    {'PASS' if quad_target else 'FAIL'} ({test_final['quadrant_accuracy']:.1%})")
    print(f"Rotation target (>85%):    {'PASS' if rot_target else 'FAIL'} ({test_final['rotation_accuracy']:.1%})")
    print(f"Rotation stretch (>90%):   {'PASS' if rot_stretch else 'FAIL'} ({test_final['rotation_accuracy']:.1%})")

    # Analyze rotation confusion (0° vs 180° and 90° vs 270°)
    print("\n" + "=" * 50)
    ...
    results: dict[str, Any] = {
        "experiment": "exp11_spatial_rotation",
        "n_train_puzzles": n_train_puzzles,
        "n_test_puzzles": n_test_puzzles,
        ...
        "rotation_head_params": rotation_params,
        ...
        "exp10_comparison": {
            ...
            "quadrant_improvement": quad_improvement,
            "rotation_improvement": rot_improvement,
        },
        "success_criteria": {
...
def main(
    ...
    if quad_acc >= 0.90:
        ...
        print(f"Position accuracy ({quad_acc:.1%}) slightly below exp9 but acceptable.")
    else:
    ...
    if rot_acc >= 0.90:
        ...
        print("Spatial rotation head successfully captures orientation cues.")
    elif rot_acc >= 0.85:
        print(f"Rotation accuracy ({rot_acc:.1%}) achieved target!")
        print("Spatial features help with rotation prediction.")
    elif rot_acc >= 0.70:
        print(f"Rotation accuracy ({rot_acc:.1%}) improved but below target.")
        print("Consider: more epochs, larger rotation head, or data augmentation.")
    else:
        print(f"Rotation accuracy ({rot_acc:.1%}) still struggling.")
        print("The spatial approach may need further refinement.")

    # Compare with exp10
    print()
    exp10_rot = 0.614
    if rot_acc > exp10_rot + 0.1:
        print(f"Major improvement over exp10 (+{rot_acc - exp10_rot:.1%})!")
        print("Spatial rotation head is the right architectural fix.")
    elif rot_acc > exp10_rot:
        print(f"Improvement over exp10 (+{rot_acc - exp10_rot:.1%}).")
    else:
        print("No improvement over exp10. Investigate further.")

    return results


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Spatial Rotation Head experiment")
    parser.add_argument(
        "--epochs",
        type=int,
        default=100,
