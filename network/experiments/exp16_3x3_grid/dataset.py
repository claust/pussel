...
- Training: Random 1 rotation per cell = 9 samples per puzzle
...
class GridRandomRotationDataset(Dataset):
    ...
    def __init__(
        ...
        print(
            ...
            f"{len(self.samples)} samples ({NUM_CELLS} per puzzle, random rotation)"
        )
...
def create_datasets(
    ...
    Training: Random rotation per sample (9 samples per puzzle)
    ...
    print(f"  Training puzzles: {len(train_ids)} " f"({len(train_ids) * NUM_CELLS} samples/epoch)")
    print(f"  Test puzzles: {len(test_ids)} " f"({len(test_ids) * NUM_CELLS * 4} samples)")
...
if __name__ == "__main__":
    print("Testing GridRandomRotationDataset for exp16 (3x3 grid)...")
