"""Siamese architecture for 3x3 grid position prediction.

Exp19: Siamese architecture with a single shared backbone for both piece and puzzle.
Instead of two separate backbones (one for piece, one for puzzle), this experiment
uses a single backbone with shared weights that processes both inputs.

Hypothesis: Weight sharing should:
- Reduce parameter count by ~50% for backbone
- Learn a common feature representation for piece/puzzle matching
- Potentially improve generalization through regularization

Key differences from exp18:
- Single shared backbone instead of dual backbones
- Both piece and puzzle features extracted with same weights
"""
...
class SiameseModel(nn.Module):
    """Siamese model with a single shared backbone for piece and puzzle.

    Uses the same backbone with shared weights to process both piece and puzzle
    images, learning a common feature space for matching. This reduces parameters
    and encourages the network to learn features useful for both inputs.
    """
    ...
    def __init__(
        ...
        """Initialize the Siamese model with a single shared backbone.
        ...
        # Create single shared backbone (Siamese architecture)
    ...
    def _extract_features(self, x: torch.Tensor) -> torch.Tensor:
        """Extract features from the shared backbone, handling different output formats."""
    ...
    def forward(self, piece: torch.Tensor, puzzle: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        """Forward pass using shared backbone for both inputs."""
        # Extract spatial features using the SAME backbone (Siamese)
        puzzle_feat_map = self._extract_features(puzzle)
        piece_feat_map = self._extract_features(piece)
...
FastBackboneModel = SiameseModel
...
if __name__ == "__main__":
    print("=" * 60)
    print("Siamese Model Test (Exp19)")
    print("=" * 60)
    ...
    for backbone_name in backbones:
        print(f"\n--- {backbone_name} (Siamese) ---")
        try:
            model = SiameseModel(backbone_name=backbone_name, pretrained=True)
