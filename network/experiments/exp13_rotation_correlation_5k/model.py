"""Dual-input regressor with ROTATION CORRELATION.

This model fixes the fundamental flaw from exp10 and exp11:
- Exp10/11: Rotation head only saw piece features, ignoring the puzzle
- Exp12: Rotation prediction compares piece features TO puzzle features

The key insight: Rotation is NOT an intrinsic property of the piece.
It's a RELATIONSHIP between piece and puzzle. The "correct" rotation
is the one where the piece best matches the corresponding puzzle region.

The fix: For each rotation candidate (0, 90, 180, 270 degrees):
1. Rotate the piece feature map
2. Compare to the puzzle region at the predicted position
3. Select the rotation with highest correlation/similarity
"""
...
class DualInputRegressorWithRotationCorrelation(nn.Module):
    ...
    This is the key fix for exp10/11's failure:
    - Position prediction uses spatial correlation (same as before)
    - Rotation prediction COMPARES piece to puzzle at each rotation

    The rotation head no longer looks at piece features in isolation.
    Instead, it tries each rotation and picks the one that best matches
    the puzzle region at the predicted position.
    """
...
if __name__ == "__main__":
    print("=" * 60)
    print("Model Architecture Test - Rotation Correlation")
    print("=" * 60)
    ...
    # Compare with exp11's rotation head size
    exp11_rotation_params = 1_001_284  # From exp11 README
    print(f"Exp11 spatial rotation head parameters: {exp11_rotation_params:,}")
    print(f"Parameter difference: {rotation_params - exp11_rotation_params:,}")
    ...
    rot_corr = RotationCorrelationModule(feature_dim=576, hidden_dim=128)
    dummy_piece_feat = torch.randn(batch_size, 576, 4, 4)
    dummy_puzzle_feat = torch.randn(batch_size, 576, 8, 8)
