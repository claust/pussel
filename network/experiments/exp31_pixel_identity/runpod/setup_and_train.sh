...
# dataset with exp30's generator (parallel, resumable) -> train with the
# unchanged exp26 recipe, but with the piece and the overview rendered as two
# independent captures, selecting the checkpoint on val and touching the
# synthetic test set once.
...
CAPTURE_PRESET="${CAPTURE_PRESET:-full}"
# Where the generated RGBA pieces live. IMPORTANT: keep this on the container
# disk (/root), NOT on /workspace — RunPod network volumes (MooseFS) charge
# ~4x quota for the ~192k small PNGs (9 GB vs the true 2.2 GB) and blow a
# 10 GB volume. The container disk is ephemeral, but generation is resumable
# (--skip-existing) and takes ~15 min; the checkpoints/results below still go
# to the persistent /workspace.
# exp31 reuses exp30's pieces byte-for-byte (everything exp31 adds happens at
# load time), so the root keeps exp30's _v2 name.
RGBA_DIR="${RGBA_DIR:-/root/realistic_4x4_rgba_v2}"

echo "========================================"
echo "exp31 RunPod Setup (preset=$CAPTURE_PRESET, epochs=$EPOCHS, batch=$BATCH_SIZE)"
echo "========================================"
...
# externally managed (PEP 668); the RunPod image's own torch lives there, so
# installing alongside it is exactly what we want.
# opencv-python-headless: data_prep.largest_component_bbox (the real path's
# bbox geometry) and exp31's rim/slop/glare stages all need cv2.
pip install --quiet --break-system-packages pillow numpy tqdm timm matplotlib opencv-python-headless
...
# Extract when the puzzles dir is missing OR empty — a leftover mountpoint or
# a previously failed extraction leaves an empty dir that must not mask a
# present tarball.
...
echo ""
echo "Generating lossless RGBA pieces (workers=$GEN_WORKERS) -> $RGBA_DIR ..."
