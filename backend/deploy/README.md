# Backend deployment (SERVER home server)

The backend runs as a Docker Compose stack on the home server, routed through
the Appwrite stack's existing Traefik at **https://pussel.example.com**.
...
under two hostnames: `pussel.example.com` (home-server-native) and
`pussel.example.net` (the URL shipped in the iOS app's Release build,
formerly pointing at Azure). Each is a proxied Cloudflare CNAME →
`example.com` in its own zone; Cloudflare terminates public TLS (SSL mode
"Full"), Traefik serves its default certificate to Cloudflare — the same
recipe as `photos.example.com` in the home-server repo.
...
```bash
# Status / logs
ssh DEPLOY_USER@SERVER 'docker ps --filter name=pussel-backend'
ssh DEPLOY_USER@SERVER 'docker logs -n 100 pussel-backend'
ssh DEPLOY_USER@SERVER 'systemctl list-timers pussel-deploy.timer'
ssh DEPLOY_USER@SERVER 'journalctl -u pussel-deploy.service -n 50'

# Manual redeploy (e.g. after editing backend.env)
ssh DEPLOY_USER@SERVER '/home/claus/pussel/repo/backend/deploy/pussel-deploy.sh --force'

# Health
curl https://pussel.example.com/health
```
